...
## Phase 2: which cases are live-execution-testable, per dialect

SQL-text parity (this harness) never touches a database, so it can't catch
execution-only bugs (real row-level locking, sequence/identity reset
semantics, actual computed values) — that's what
`drivers/<dialect>/test/integration/` is for. `tool/parity/EXECUTABLE_CASES.md`
(generated by `node tool/parity/tag_executable_cases.mjs`, regenerate after
any corpus change) lists, per dialect, every case id that compiles to real
SQL there (vs. one knex.js itself refuses, which has nothing to execute) —
the target roadmap for live-execution test coverage. Checkboxes in that file
are static, not a live tracking mechanism — there's no automated way to tell
whether a given case id already has a live test.
