...
@client
final class MyGitHubButton extends StatefulComponent {
  /// Create a new [MyGitHubButton] component that links to the specified GitHub [repo]
  /// and shows the amount of stars and forks it has.
  const MyGitHubButton({required this.repo, super.key});
  ...
  @override
  State<MyGitHubButton> createState() => _GitHubButtonState();
    ...
    css('.github-button', [
      ...
      css('.github-info', [
        ...
        css('& > span:first-child', [
          // Default (Desktop): Hide mobile, show desktop
          css('.mobile-only-repo').styles(display: Display.none),
          css('.desktop-only-repo').styles(display: Display.block),
        ]).styles(
...
class _GitHubButtonState extends State<MyGitHubButton> with PreloadStateMixin<MyGitHubButton> {
  int? _stars;
  int? _forks;
  ...
  Component build(BuildContext _) {
    return div(
      styles: Styles(display: Display.contents),
      [
        // Responsive styles for repo name using raw HTML to ensure correct media query syntax
        div(
          ...
          [
            ...
            <style>
              @media (max-width: 600px) {
                .github-button .mobile-only-repo { display: block !important; }
                .github-button .desktop-only-repo { display: none !important; }
              }
            </style>
          '''),
          ],
        ),
        a(href: 'https://github.com/${component.repo}', target: Target.blank, classes: 'github-button not-content', [
          div(classes: 'github-icon', const [_GitHubIcon()]),
          div(classes: 'github-info', [
            span([
              // Responsive repo name:
              // Mobile: repo name only (e.g. "fletch")
              // Desktop: full name (e.g. "kartikey321/fletch")
              span(classes: 'mobile-only-repo', [Component.text(component.repo.split('/').last)]),
              span(classes: 'desktop-only-repo', [Component.text(component.repo)]),
            ]),
            span([
              Component.text('★'),
