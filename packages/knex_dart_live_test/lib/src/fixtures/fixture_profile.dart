...
const schemaDdlEmptyV1 = FixtureProfileRef(
  'schema_ddl_empty_v1',
  'no prerequisite — self-contained CREATE-type cases',
);
const schemaDdlUsersBareV1 = FixtureProfileRef(
  'schema_ddl_users_bare_v1',
  'bare users table, zero columns — pure ADD-column cases',
);
const schemaDdlUsersConstraintsV1 = FixtureProfileRef(
  'schema_ddl_users_constraints_v1',
  'users(foo, test1, test2, email) — ADD-constraint-over-existing-column cases',
);
const schemaDdlUserSingularV1 = FixtureProfileRef(
  'schema_ddl_user_singular_v1',
  "table literally named 'user' (singular)",
);
const schemaDdlUsersNicknameV1 = FixtureProfileRef(
  'schema_ddl_users_nickname_v1',
  'users(nickname) — drop/rename-column cases',
);
const schemaDdlUsersFooBarV1 = FixtureProfileRef(
  'schema_ddl_users_foo_bar_v1',
  'users(foo, bar) — drop-columns-multiple',
);
const schemaDdlUsersNicknameAvatarV1 = FixtureProfileRef(
  'schema_ddl_users_nickname_avatar_v1',
  'users(nickname, avatar) — drop-columns-multi',
);
const schemaDdlUsersTimestampsV1 = FixtureProfileRef(
  'schema_ddl_users_timestamps_v1',
  'users(created_at, updated_at) — drop-timestamps',
);
const schemaDdlUsersEmailV1 = FixtureProfileRef(
  'schema_ddl_users_email_v1',
  'users(email), nullable — drop-nullable (SET NOT NULL)',
);
const schemaDdlUsersEmailNotNullV1 = FixtureProfileRef(
  'schema_ddl_users_email_not_null_v1',
  'users(email) NOT NULL — set-nullable (DROP NOT NULL), so the statement has a real effect to prove',
);
const schemaDdlUsersUniqueDefaultV1 = FixtureProfileRef(
  'schema_ddl_users_unique_default_v1',
  'users(email) with default-named unique constraint — drop-unique (unnamed)',
);
const schemaDdlUsersUniqueNamedV1 = FixtureProfileRef(
  'schema_ddl_users_unique_named_v1',
  'users(email) with a named unique constraint — drop-unique-named',
);
const schemaDdlUsersIndexDefaultV1 = FixtureProfileRef(
  'schema_ddl_users_index_default_v1',
  'users(email) with default-named index — drop-index (unnamed)',
);
const schemaDdlUsersIndexNamedV1 = FixtureProfileRef(
  'schema_ddl_users_index_named_v1',
  'users(email) with a named index — drop-index-named',
);
const schemaDdlUsersUniqueFooV1 = FixtureProfileRef(
  'schema_ddl_users_unique_foo_v1',
  "users with a unique constraint literally named 'foo' — dropUnique(null, 'foo')",
);
const schemaDdlUsersIndexFooV1 = FixtureProfileRef(
  'schema_ddl_users_index_foo_v1',
  "users with an index literally named 'foo' — dropIndex(null, 'foo')",
);
const schemaDdlUsersForeignFooV1 = FixtureProfileRef(
  'schema_ddl_users_foreign_foo_v1',
  "users with an FK literally named 'foo' — dropForeign(null, 'foo')",
);
const schemaDdlUsersPrimaryNamedV1 = FixtureProfileRef(
  'schema_ddl_users_primary_named_v1',
  "users with a PK named 'testconstraintname' — drop-primary-named",
);
const schemaDdlMembershipsBareV1 = FixtureProfileRef(
  'schema_ddl_memberships_bare_v1',
  'memberships(user_id, org_id), no PK — add-unique/index-composite, primary',
);
const schemaDdlMembershipsPkV1 = FixtureProfileRef(
  'schema_ddl_memberships_pk_v1',
  'memberships(user_id, org_id) with a composite PK — drop-primary',
);
const schemaDdlOrdersBareV1 = FixtureProfileRef(
  'schema_ddl_orders_bare_v1',
  'users + orders, no FK yet — add-column-foreign',
);
const schemaDdlOrdersFkV1 = FixtureProfileRef(
  'schema_ddl_orders_fk_v1',
  'users + orders with a default-named FK — drop-foreign',
);
const schemaDdlOrdersUserIdV1 = FixtureProfileRef(
  'schema_ddl_orders_user_id_v1',
  'users + orders(user_id), no FK yet — foreign, foreign-both-actions',
);
const schemaDdlTBareV1 = FixtureProfileRef(
  'schema_ddl_t_bare_v1',
  "table literally named 't' — alter-table-column-unsigned",
);
const schemaDdlCompositeKeyTestV1 = FixtureProfileRef(
  'schema_ddl_composite_key_test_v1',
  'composite_key_test(column_a, column_b) with a matching unique constraint — drop-unique-composite',
);
const schemaDdlUsersRefV1 = FixtureProfileRef(
  'schema_ddl_users_ref_v1',
  'users(id) as an FK/reference target — create-table-foreign-*, drop-table, rename-table',
);
const schemaDdlUsersAccountsRefV1 = FixtureProfileRef(
  'schema_ddl_users_accounts_ref_v1',
  'users(id) + accounts(id) as FK targets — create-table-foreign-mixed-actions',
);
const schemaDdlUsersNameRefV1 = FixtureProfileRef(
  'schema_ddl_users_name_ref_v1',
  'users(id, name) — createTableLike source',
);
const schemaDdlUsersActiveRefV1 = FixtureProfileRef(
  'schema_ddl_users_active_ref_v1',
  'users(id, name, active) — create-view-bare',
);
const schemaDdlUsersAgeRefV1 = FixtureProfileRef(
  'schema_ddl_users_age_ref_v1',
  'users(id, name, age) — view-create-basic/or-replace/materialized',
);
const schemaDdlBillingSchemaV1 = FixtureProfileRef(
  'schema_ddl_billing_schema_v1',
  "schema 'billing' pre-existing — drop-schema family",
);
const schemaDdlActiveUsersMvV1 = FixtureProfileRef(
  'schema_ddl_active_users_mv_v1',
  "materialized view 'active_users_mv' pre-existing",
);
const schemaDdlActiveUsersMvConcurrentV1 = FixtureProfileRef(
  'schema_ddl_active_users_mv_concurrent_v1',
  "materialized view 'active_users_mv' + unique index — refresh ... concurrently",
);
const schemaDdlViewToRefreshV1 = FixtureProfileRef(
  'schema_ddl_view_to_refresh_v1',
  "materialized view 'view_to_refresh' pre-existing",
);
const schemaDdlViewToRefreshConcurrentV1 = FixtureProfileRef(
  'schema_ddl_view_to_refresh_concurrent_v1',
  "materialized view 'view_to_refresh' + unique index — refresh ... concurrently",
);
const schemaDdlViewUsersV1 = FixtureProfileRef(
  'schema_ddl_view_users_v1',
  "a VIEW literally named 'users' — view-drop",
);
const schemaDdlViewOldV1 = FixtureProfileRef(
  'schema_ddl_view_old_v1',
  "view 'old_view' pre-existing — view-rename",
);
const schemaDdlViewActiveUsersV1 = FixtureProfileRef(
  'schema_ddl_view_active_users_v1',
  "view 'active_users' pre-existing — drop-view-if-exists",
);
const schemaDdlMyschemaUsersTableV1 = FixtureProfileRef(
  'schema_ddl_myschema_users_table_v1',
  "schema 'myschema' + table myschema.users — drop-table(-if-exists)-with-schema",
);
const schemaDdlMyschemaUsersViewV1 = FixtureProfileRef(
  'schema_ddl_myschema_users_view_v1',
  "schema 'myschema' + view myschema.users — view-drop-with-schema",
);
const schemaDdlMyschemaCapUsersIndexV1 = FixtureProfileRef(
  'schema_ddl_myschema_cap_users_index_v1',
  "schema 'mySchema' + table + index — alter-table-drop-index-with-schema",
);
const schemaDdlCitextUsersV1 = FixtureProfileRef(
  'schema_ddl_citext_users_v1',
  'citext extension + bare users table — column-specifictype-unique-notnull',
);
