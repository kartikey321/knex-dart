...
const Map<String, Map<String, String>> unsupportedEngineAllowlist = {
  'postgres': {
    ...
    'delete/join-multi':
        ...
        'FROM-clause entry for table \\"user\\""). This looks like a '
        'singular/plural mismatch in the case itself (likely meant '
        '"users.email") rather than an engine limitation — flagged here '
        'rather than silently "fixed" by editing the shared corpus, since '
        'that decision belongs to whoever owns the corpus, not this '
        'fixture-linking pass.',
    'delete/join-single':
        'Same singular/plural mismatch as delete/join-multi ("user.email" '
        'vs the actual "users" table) — same flag: looks like a likely '
        'corpus typo, not fixed here.',
    'subquery/select-first-as':
        ...
        '"e.dept_no" and fails to cast it (22P02). Looks like a likely '
        'where()-vs-whereColumn() mistake in the case itself, not an '
        'engine limitation — flagged here rather than silently "fixed" by '
        'editing the shared corpus.',
    'subquery/select-scalar':
        'Same outer where()-vs-whereColumn() issue as subquery/select-'
        'first-as (comparing dept_no against the literal string '
        '"e.dept_no"), plus a inner `.select([\'avg(salary)\'])` passing a '
        'plain string where an aggregate expression was likely intended '
        '(would need .select([raw(\'avg(salary)\')]) to compile as a '
        'function call rather than a quoted identifier). Two likely '
        'authoring issues in the case itself, not fixed here.',
