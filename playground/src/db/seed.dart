...
void _schema(List<Map<String, dynamic>> stmts) {
  for (final s in stmts) {
    _sql(
      s['sql'] as String,
      (s['bindings'] as List?)?.cast<dynamic>() ?? const [],
    );
  }
}
...
void main() {
  ...
  final dialectStr = const String.fromEnvironment(
    'DIALECT',
    defaultValue: 'sqlite',
  );
  final dialect = dialectStr == 'postgres'
  ...
  _schema(
    db.schemaBuilder().createTableIfNotExists('users', (t) {
      t.increments('id');
  ...
  _schema(
    db.schemaBuilder().createTableIfNotExists('orders', (t) {
      t.increments('id');
